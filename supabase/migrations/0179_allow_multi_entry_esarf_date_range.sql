-- Migration 0179: Allow multi-entry ESARF requests to span multiple days while preserving 1-2 consecutive days rule for single-entry requests
...
begin
  ...
  if exists (select 1 from public.time_request_details where request_id = p_request_id) then
    ...
    -- Multi-entry ESARF requests (entries serialized in reason with '[Entry 1]...')
    -- have overall date_from and date_to that span across all entries, which can legitimately exceed 2 days.
